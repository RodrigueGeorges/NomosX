...
export default async function SourcePage({ params }: { params: { id: string } }) {
  ...
  if (!s) return <Shell><h1 className="text-3xl">Source not found</h1></Shell>;

  return (
    <Shell>
      <div className="flex items-center justify-between flex-wrap gap-2">
        <h1 className="text-3xl font-semibold leading-tight">{s.title}</h1>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-8">
        <Card className="lg:col-span-2">
          <CardHeader><h2 className="text-lg font-semibold">Abstract</h2></CardHeader>
          <CardContent><p className="text-sm text-muted whitespace-pre-wrap">{s.abstract ?? "—"}</p></CardContent>
        </Card>
        <Card>
          <CardHeader><h2 className="text-lg font-semibold">Metadata</h2></CardHeader>
