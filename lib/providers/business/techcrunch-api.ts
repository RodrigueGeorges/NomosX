  ...
  try {
    ...
    const feedPromises = rssFeeds.map(async (feedUrl, index) => {
      try {
        ...
        console.warn(`[TechCrunch] Feed ${index} error: ${error.message}`);
        return [];
      }
    });
    ...
    console.error('[TechCrunch] Primary search failed:', error.message);
...
function parseTechCrunchRSS(xmlText: string, feedIndex: number): any[] {
  ...
  itemMatches.forEach((itemXML, index) => {
    try {
      ...
      console.warn(`[TechCrunch] Parse error item ${index}:`, error.message);
    }
  });
