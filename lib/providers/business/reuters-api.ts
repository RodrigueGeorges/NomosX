  ...
  try {
    ...
    const feedPromises = rssFeeds.map(async (feedUrl, index) => {
      try {
        ...
        console.warn(`[Reuters] Feed ${index} error: ${error.message}`);
        return [];
      }
    });
    ...
    console.error('[Reuters] Primary search failed:', error.message);
...
function parseReutersRSS(xmlText: string, feedIndex: number): any[] {
  ...
  itemMatches.forEach((itemXML, index) => {
    try {
      ...
      console.warn(`[Reuters] Parse error item ${index}:`, error.message);
    }
  });
