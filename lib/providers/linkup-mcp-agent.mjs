...
import { linkUpClient, LinkUpSearchSchema, LinkUpResultSchema } from './linkup-provider.mjs';
import { z } from 'zod';

// ===== SCHEMA D'INTÉGRATION MCP =====
export const LinkUpMCPRequestSchema = z.object({
...
export class LinkUpMCPAgent {
  ...
  private async deepAnalyze(request: LinkUpMCPRequest): Promise<any> {
    ...
    const analysis = {
      ...
      gaps: await this.identifyGaps(results),
  ...
  private async contextualEnrichment(params: any, context: any): Promise<any> {
  ...
  private async semanticAnalysis(results: any[], context: any): Promise<any[]> {
  ...
  private async mcpScoring(results: any[], context: any): Promise<any[]> {
  ...
  private calculateMCPScore(result: any, context: any): number {
    let score = 0;
  ...
  private async generateInsights(results: any[], context: any): Promise<any> {
    const insights = [];
  ...
  // ===== MÉTHODES D'ANALYSE =====
  private async thematicAnalysis(results: any[]): Promise<any> {
  ...
  private async temporalAnalysis(results: any[]): Promise<any> {
    const dates = results
  ...
  private async qualityAnalysis(results: any[]): Promise<any> {
  ...
  private extractDominantTopics(results: any[]): string[] {
  ...
  private async establishConsensus(results: any[]): Promise<any> {
  ...
  private calculateComplementarityIndex(results: any[]): number {
    if (results.length === 0) return 0;
  ...
  private calculateQualityImprovement(original: any[], enriched: any[]): number {
  ...
  private identifyQualityFactors(results: any[]): string[] {
    const factors = [];
    
    const hasDOI = results.some(r => r.metadata?.doi);
    if (hasDOI) factors.push('DOI presence');
    
    const hasCitations = results.some(r => r.metadata?.citations > 0);
  ...
  private buildComplementarySearch(originalParams: any, gaps: string[]): any {
  ...
  private async scoreComplementarity(complementary: any[], existing: any[]): Promise<any[]> {
  ...
  private calculateComplementarityScore(comp: any, existing: any[]): number {
  ...
  private async createIntegrationPlan(complementary: any[], existing: any[]): Promise<any> {
    return {
