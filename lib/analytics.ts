  ...
  try {
    ...
    if (event.includes('payment') || event.includes('purchase')) {
      ...
      revenue = revenueEvents.reduce(
        (sum, event) => sum + (event.properties?.amount || 0),
        0
      )
    }
...
export async function getTrafficMetrics(period: {
  ...
  try {
    ...
    // Sources de trafic
    const trafficSources = sessions.reduce((sources, session) => {
    ...
    const topPages = await Promise.all(
      pageViewsByPath.map(async (page) => {
        ...
        const uniqueViews = new Set(pageData.map(pv => pv.sessionId)).size
